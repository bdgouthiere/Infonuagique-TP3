...
namespace ModernRecrut.MVC.Controllers
{
    public class PostulationsController : Controller
    {
        ...
        public async Task<ActionResult> Postuler(RequetePostulation requetPostulation)
        {
            ...
            bool cvPresent = documents?.Any(d => d.Contains($"{requetPostulation.CandidatId}_CV_")) ?? false;
            if (!cvPresent)
            ...
            bool lettreMotivationPresent = documents?.Any(d => d.Contains($"{requetPostulation.CandidatId}_LettreDeMotivation_")) ?? false;
